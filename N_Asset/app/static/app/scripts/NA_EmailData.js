                    ...
                    if (is_emplChecked.prop('checked') == false && is_suplChecked.prop('checked') == false) {
                        ...
                        searchInput.autocomplete({
                            source: function (request, response) {
                                $.ajax({
                                    url: "http://localhost:8000/NA_EmailData/searchBiodata/",
                                    method: 'POST',
                                    data: {
        ...
        $('div.dialogButtonOKCancel>a.button:eq(0)').click(function (event) {
            ...
            if (!emailForm.checkValidity()) {
                ...
                if ($('span#successConvert').length == 0) {
                    ...
                    $.ajax({
                        url: "http://localhost:8000/NA_EmailData/",
                        method: "POST",
                        data: send_mailData(),
        ...
        $('div.bottomDialogContent').find('a.btn-link').click(function (event) {
            ...
            if (is_selectData.val() == 'tabular_data') {
                if (is_emplChecked.prop('checked') == false && is_suplChecked.prop('checked') == false) {
                    ...
                    if (is_emplChecked.prop('checked') == true) {
                        $.ajax({
                            url: 'http://localhost:8000/NA_Employee/getData/?columnName=employee_name&valueKey=&dataType=Varchar&criteria=like&_search=false&nd=1517129972488&rows=100&page=1&sidx=idapp&sord=desc',
                            dataType: 'json',
                            ...
                            success: function (data) {
                                ...
                                if (target.innerText.trim() == 'Export To PDF') {
                                    ...
                                    $.ajax({
                                        url: 'http://localhost:8000/NA_EmailUplData/',
                                        method: 'POST',
                                        beforeSend: function (jqXHR) {
                    ...
                    if (is_suplChecked.prop('checked') == true) {
                        $.ajax({
                            url: 'http://localhost:8000/NA_Suplier/getData/?columnName=supliername&valueKey=&dataType=Varchar&criteria=like&_search=false&nd=1517132750079&rows=20&page=1&sidx=createddate&sord=desc',
                            dataType: 'json',
                            beforeSend: function (jqXHR) {
                                var csrf_token = get_csrf();
                ...
                if (is_emplChecked.prop('checked') == false && is_suplChecked.prop('checked') == false) {
                    ...
                    $.ajax({
                        url: "http://localhost:8000/NA_Email/retriveBio/",
                        method: "POST",
                        beforeSend: function (jqXHR) {
                            jqXHR.setRequestHeader('X-CSRF-Token', get_csrf())
                        },
